...
public class Arguments : IEnumerable<KeyValuePair<string, string>>, ISerializable
{
    ...
    {
        Regex splitter = new("^-{1,2}|^/|=|:", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        ...
        foreach (string arg in args)
        {
            ...
            if (parameter is not null) 
                m_arguments.TryAdd(parameter, null);
    ...
    public virtual string? this[string argument]
    {
        get
        {
            m_arguments.TryGetValue(argument, out string? value);
            return value;
        }
    }
    ...
    // Static Fields
    private static readonly Regex s_compiledTokenRegex = new(TokenRegex, RegexOptions.Compiled);
    ...
    {
        lock (s_compiledTokenRegex)
            return s_compiledTokenRegex.Matches(command)
    ...
    public static string[] ToArgs(string command)
    {
        ...
        string unescape(string escapeSequence)
        {
            return escapeSequence switch
            {
                @"\n" => "\n",
                @"\r" => "\r",
                @"\t" => "\t",
                _ => escapeSequence.Substring(1)
            };
        }
    ...
    {
        StringBuilder escapedArgBuilder = new();
