...
#pragma warning disable CS0649, CS1591, VSSpell001
...
{
    // 1) Only expose Windows API methods as needed.
    // 2) Where possible, use [LibraryImport] instead of [DllImport] for NET (.NET 5.0 and later) - .NET Standard still uses [DllImport].
    // 3) Mark all structures and classes with the [NoReorder] attribute to prevent reordering of fields by Resharper.
    // 4) Group related methods, structures and enumerations together (if possible) and comment where functions are referenced
    // 5) Consider cross-platform compatibility needs when declaring Windows API methods, see Gemstone.POSIX for POSIX API methods
    ...
    */
    ...
    /// </summary>
#if NET
    [LibraryImport("Kernel32", StringMarshalling = StringMarshalling.Utf16, EntryPoint = "FormatMessageW", SetLastError = true)]
    public static partial int FormatMessage(int dwFlags, ref IntPtr lpSource, int dwMessageId, int dwLanguageId, ref string lpBuffer, int nSize, ref IntPtr Arguments);
#else
    ...
    //
    // Following API functions reference by Gemstone.Common:
    //
    ...
    //
    // Following API functions reference by Gemstone.IO.FilePath:
    //
    ...
    [DllImport("mpr", EntryPoint = "WNetAddConnection2W", ExactSpelling = true, CharSet = CharSet.Unicode, SetLastError = true)]
