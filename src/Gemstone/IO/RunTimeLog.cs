...
public class RunTimeLog : ISupportLifecycle, IProvideStatus
{
    ...
    public virtual string Status
    {
        get
        {
            ...
            status.AppendLine($"    Run-time log file name: {FilePath.TrimFileName(m_fileName.ToNonNullString("undefined"), 51)}");
            status.AppendLine($"  Auto-flush timer enabled: {Enabled}");
            ...
            lock (m_readerWriterLock)
            {
                if (File.Exists(m_fileName))
                {
                    ...
                    if (fileData.Length > 0)
                    {
                        ...
                        if (!settings.TryGetValue(LastStartTimeKey, out string? setting) || !DateTimeOffset.TryParseExact(setting.Trim(), DateTimeFormat, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AllowInnerWhite, out m_startTime))
