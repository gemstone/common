...
namespace gemstone.threading
{
    ...
    public class ScheduledTask : IDisposable
    {
        ...
        /// </summary>
        internal void IgnoreShutdownEvent() => m_thread.IgnoreShutdownEvent();
        ...
        /// </remarks>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Start(int delay) => m_thread.Start(delay);
        ...
        private void TryCallback(ScheduledTaskRunningReason args)
        {
            ...
            catch (Exception ex)
            {
                try
                {
                    UnhandledException?.Invoke(this, new EventArgs<Exception>(ex));
                }
                catch (Exception ex2)
                {
